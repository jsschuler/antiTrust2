...
# generate a seed 
seed1Vec=sort(repeat(rand(DiscreteUniform(1,10000),sweeps),3*reps))
seed2Vec=rand(DiscreteUniform(1,10000),3*sweeps*reps)
...
pctConnectedVec=repeat([.2,.2,.2],sweeps*reps)
expDegreeVec=floor.(Int64,pctConnectedVec.*agtCntVec)
#βVec=sort(repeat(rand(Uniform(),sweeps),reps))
βVec=repeat([.5,.5,.5],sweeps*reps)
# Finally, we need a Poisson parameter to how much agents search
#searchQtyVec=sort(repeat(rand(Uniform(100,100),sweeps),reps))
searchQtyVec=repeat([100,100,100],sweeps*reps)
modRunVec=repeat([500,500,500],sweeps*reps)

currTime=now()

ctrlFrame=DataFrame()
ctrlFrame[!,"dateTime"]=repeat([currTime],3*sweeps*reps)
ctrlFrame[!,"seed1"]=seed1Vec
ctrlFrame[!,"seed2"]=seed2Vec
ctrlFrame[!,"key"]=string.(repeat([currTime],3*sweeps*reps)).*"-".*string.(seed1Vec) .*"-".*string.(seed2Vec).*"-"
